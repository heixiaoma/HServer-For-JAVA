...
import net.hserver.core.interfaces.TrackAdapter;
...
public class MemoryInitClass {
    ...
    public static void init(String packageName) {
        //自己不跟踪
        if (packageName == null || packageName.startsWith("net.hserver")) {
            return;
        }
        try {
            ...
            try {
                ...
                declaredMethod.addLocalVariable("annMethodObj", cp.get(CtMethod.class.getCanonicalName()));
                 ...
                 String before = "begin_hserver=System.currentTimeMillis();" +
                        "spanId=net.hserver.core.server.util.SpanUtil.get();" +
                        "pspanId=net.hserver.core.server.util.SpanUtil.add();" +
                        "annMethodObj = (javassist.CtMethod)net.hserver.core.ioc.ref.MemoryInitClass.annMapMethod.get(\"" + uuid + "\");";
                ...
                src.append("trackAdapter_hserver = net.hserver.core.ioc.IocUtil.getListBean(net.hserver.core.interfaces.TrackAdapter.class);");
                ...
                src.append("{");
                src.append(" ((net.hserver.core.interfaces.TrackAdapter)trackAdapter_hserver.get(i)).track(clazz_hserver,annMethodObj,Thread.currentThread().getStackTrace(), begin_hserver,end_hserver,pspanId,spanId);");
                src.append("}");
                src.append("}");
                src.append("net.hserver.core.server.util.SpanUtil.clear();");
